...
bundle_name = dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get().split("/.bundle/")[1].split("/")[0] if "/.bundle/" in dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get() else "agentops_dev"
target = dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get().split("/.bundle/")[1].split("/")[1] if "/.bundle/" in dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get() else "dev"
default_bundle_root = f"/Workspace/Users/{current_user}/.bundle/{bundle_name}/{target}/files"
dbutils.widgets.text(
    "bundle_root",
    default_bundle_root,
...
from agent_development.agent_evaluation.utils.evaluation import get_reference_documentation
...
def evaluate_model(question):
    return model.predict({"input": [{"role": "user", "content": question}]})
